...
namespace MAX.Drawing.Brushes
{
    public class CloudyBrushFactory : BrushFactory
    {
        ...
        public static bool Handler(string arg, Player p, ref NoiseArgs args)
        {
            ...
            if (opt == 'o')
            {
                if (!OrderParser.GetInt(p, arg, "Octaves", ref args.Octaves, 1, 16)) return false;
            }
            else if (opt == 's')
            {
                if (!OrderParser.GetInt(p, arg, "Seed", ref args.Seed)) return false;
            }
            else
            {
        ...
        {
            if (!OrderParser.GetReal(p, arg, "Value", ref target)) return false;
