...
namespace MAX.Drawing.Brushes
{
    ...
    public static class FrequencyBrush
    {
                                     ...
                                     Predicate<string> argHandler)
        {
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                ...
                int count = OrderParser.GetBlocks(p, arg, blocks, true);
                ...
                if (sepIndex >= 0)
                {
                    ...
                    if (!OrderParser.GetInt(p, arg, "Frequency", ref freq, 1, 1000)) return false;
                }
            ...
            foreach (ushort b in blocks)
            {
                ...
                if (!OrderParser.IsBlockAllowed(p, "draw with", b)) return false;
            }
            return true;
        }
    ...
    public class GradientBrushFactory : BrushFactory
    {
        ...
        // TODO: Need to unify axis parsing code across MAX
