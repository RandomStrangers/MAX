...
namespace MAX.Drawing.Brushes 
{
    public sealed class SolidBrushFactory : BrushFactory 
    {
        ...
        public override Brush Construct(BrushArgs args) {
            ...
            BlockID block;
            if (!OrderParser.GetBlockIfAllowed(p, args.Message, "draw with", out block)) return null;
        ...
        public override bool Validate(BrushArgs args) {
            ...
            BlockID block;
            return OrderParser.GetBlockIfAllowed(args.Player, args.Message, "draw with", out block);
        }
    }
    ...
    public sealed class StripedBrushFactory : BrushFactory 
    {
        ...
        public override Brush Construct(BrushArgs args) {
            ...
            BlockID block1;
            if (!OrderParser.GetBlockIfAllowed(p, parts[0], "draw with", out block1, true)) return null;
            if (parts.Length == 1)
                return new StripedBrush(block1, Block.Invalid);
            
            BlockID block2;
            if (!OrderParser.GetBlockIfAllowed(p, parts[1], "draw with", out block2, true)) return null;
