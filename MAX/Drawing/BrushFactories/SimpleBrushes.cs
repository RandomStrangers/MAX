...
namespace MAX.Drawing.Brushes
{
    public class SolidBrushFactory : BrushFactory
    {
        ...
        public override Brush Construct(BrushArgs args)
        {
            ...
            {
                if (!OrderParser.IsBlockAllowed(p, "draw with", args.Block)) return null;
                return new SolidBrush(args.Block);
            }

            if (!OrderParser.GetBlockIfAllowed(p, args.Message, "draw with", out ushort block)) return null;
        ...
        public override bool Validate(BrushArgs args)
        {
            ...
            return OrderParser.GetBlockIfAllowed(args.Player, args.Message, "draw with", out ushort _);
        }
    }
    ...
    public class StripedBrushFactory : BrushFactory
    {
        ...
        public override Brush Construct(BrushArgs args)
        {
            ...
            if (!OrderParser.GetBlockIfAllowed(p, parts[0], "draw with", out ushort block1, true)) return null;
            if (parts.Length == 1)
                return new StripedBrush(block1, Block.Invalid);

            if (!OrderParser.GetBlockIfAllowed(p, parts[1], "draw with", out ushort block2, true)) return null;
