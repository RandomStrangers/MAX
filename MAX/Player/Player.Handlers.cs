...
namespace MAX
{
    public partial class Player : IDisposable
    {
        ...
        public ChangeResult PlaceBlock(ushort x, ushort y, ushort z, ushort block)
        {
        ...
        public void ProcessChat(string text, bool continued)
        {
            ...
            text = Chat.ParseInput(text, out bool isOrder);
        ...
        public void DoOrder(string text)
        {
            ...
            text.Separate(' ', out string ord, out string args);
        ...
        public void HandleOrder(string ord, string args, OrderData data)
        {
            ...
            try
            {
                ...
                try { thread.Name = "ORD_" + ord; } catch { }
        ...
        public Order GetOrder(ref string ordName, ref string ordArgs, OrderData data)
        {
            ...
            if (OrdBindings.TryGetValue(ordName, out string bound))
            {
                // user defined order shortcuts take priority
                bound.Separate(' ', out ordName, out ordArgs);
            }
            else if (byte.TryParse(ordName, out byte bindIndex) && bindIndex < 10)
            {
            ...
            if (jailed && !order.UseableWhenJailed)
            {
