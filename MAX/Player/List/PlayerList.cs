...
namespace MAX
{
    ...
    public class PlayerList
    {
        ...
        public void Output(Player p, string group, string listOrd, string modifier)
        {
            ...
            else
            {
                ...
                Paginator.Output(p, list, (name) => p.FormatNick(name),
                                 listOrd, "players", modifier);
            }
        }

        /// <summary> Outputs list of players using MultiPageOutput.Output. </summary>
        /// <remarks> Names are not formatted at all. </remarks>
        public void OutputPlain(Player p, string group, string listOrd, string modifier)
        {
            List<string> list = All();
            if (list.Count == 0)
            {
