...
namespace MAX.Eco
{
    public class LevelItem : Item
    {
        ...
        public override void OnPurchase(Player p, string raw)
        {
            ...
            if (raw.Length == 0) { OnStoreOrder(p); return; }
            LevelPreset preset = FindPreset(args[0]);
        ...
        public void AddPreset(Player p, string[] args, LevelPreset preset)
        {
            ...
            preset.type = args[6];
            if (!OrderParser.GetInt(p, args[7], "Price", ref preset.price, 0)) return;
        ...
        public void EditPreset(Player p, string[] args, LevelPreset preset)
        {
            ...
            {
                int newPrice = 0;
                if (!OrderParser.GetInt(p, args[4], "Price", ref newPrice, 0)) return;
