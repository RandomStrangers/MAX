...
namespace MAX 
{
    public static class SrvProperties 
    {
        ...
        public static void SetOldReview() {
            ...
            if (old.nextPerm != -1)
                OrderExtraPerms.Find("Review", 2).MinRank = (LevelPermission)old.nextPerm;
            if (old.clearPerm != -1)
        ...
        public static void SaveProps(StreamWriter w) {
            ...
            w.WriteLine("#   agree-to-rules-on-entry       = Forces all new players to the server to agree to the rules before they can Build or use orders.");
