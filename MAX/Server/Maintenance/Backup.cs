...
namespace MAX
{
    ...
    {
        public const string zipPath = "MAX.zip", sqlPath = "SQL.sql";
        ...
        public static void BackupDatabase(StreamWriter sql)
        {
            ...
            sql.WriteLine("-- {0} SQL database dump", Server.NameVersioned);
        ...
        public static void ImportBulk(StreamReader reader)
        {
            ...
            try
            {
                ...
                {
                    string ord = NextStatement(reader, buffer);
                    if (ord == null || ord.Length == 0) continue;

                    int index = ord.ToUpper().IndexOf("CREATE TABLE");
                    if (index >= 0)
                    {
                        ord = ord.Remove(0, index);
                        ord = ord.Replace(" unsigned", " UNSIGNED");
                        Database.Backend.ParseCreate(ref ord);
                    }

                    //Run the order in the transaction.
                    if (bulk.Execute(ord, null)) continue;
