...
namespace MAX.Levels.IO
{
    ...
    public struct PackedCol : IEquatable<PackedCol>
    {
        ...
        {
            xSide = Scale(normal, ShadeX);
            zSide = Scale(normal, ShadeZ);
            yBottom = Scale(normal, ShadeYBottom);
        }
        ...
        {
            return (obj is PackedCol col) && Equals(col);
        }
        public bool Equals(PackedCol other)
        {
        ...
        public static bool TryParse(string input, out PackedCol value)
        {
            ...
            int i = input[0] == '#' ? 1 : 0;
            if (!UnHex(input[i + 0], out int rH) || !UnHex(input[i + 1], out int rL))
            {
                return false;
            }
            if (!UnHex(input[i + 2], out int gH) || !UnHex(input[i + 3], out int gL))
            {
                return false;
            }
            if (!UnHex(input[i + 4], out int bH) || !UnHex(input[i + 5], out int bL))
            {
                return false;
            }
        ...
        {
            if (!TryParse(input, out PackedCol value))
            {
    ...
    public class NbtFile2
    {
        ...
        public void Write(NbtTagType v, string name) 
        {
            writer.Write((byte)v); 
            Write(name); 
        }
        ...
        public void WriteBytes(byte[] v, int index, int count) 
        {
            writer.Write(v, index, count); 
        }
        ...
        public unsafe NbtTag2 ReadTag(byte typeId, bool readTagName)
        {
            ...
            switch ((NbtTagType)typeId)
            {
                ...
                case NbtTagType.Float:
                    ...
                    tag.Value = *(float*)&temp32; 
                    break;
                case NbtTagType.Double:
                    long temp64 = ReadInt64();
                    tag.Value = *(double*)&temp64;
                    break;
                case NbtTagType.ByteArray:
                    tag.Value = reader.ReadBytes(ReadInt32()); 
                    break;
                ...
                //case NbtTagType.List:
                    ...
                    //{
                        //ChildTagId = (NbtTagType)reader.ReadByte(),
                        //ChildrenValues = new object[ReadInt32()]
                    //};
                ...
                case NbtTagType.List:
                    ...
                    {
                        ListType = (NbtTagType)reader.ReadByte()
                    };
                    NbtTag[] tags = new NbtTag[ReadInt32()];
                    list.Tags.AddRange(tags);
                    for (int i = 0; i < list.Tags.Count; i++)
                    {
                        list.Tags[i] = (NbtTag)ReadTag((byte)list.ListType, false).Value;
                    }
                    tag.Value = list;
                    break;
                case NbtTagType.Compound:
                    ...
                    {
                        NbtTag2 child = ReadTag(childTagId, true); 
    ...
    public unsafe class CwExporter : IMapExporter
    {
        ...
        public void WriteSpawnCompoundTag(Level lvl)
        {
        ...
        public void WriteMetadata(Level lvl)
        {
            ...
            string url = lvl.Config.TexturePack ?? "";
            nbt.Write(url);
            nbt.Write(NbtTagType.End);
        ...
        public void WriteColCompound(string name, PackedCol col)
        {
        ...
        public unsafe void WriteBlockDefinitionCompound(Level lvl, byte id)
        {
            nbt.Write(NbtTagType.Compound, "Block" + id);
            BlockDefinition def = lvl.GetBlockDef(id) ?? BlockDefinition.GlobalDefs[id];
            if (def == null)
            {
                return;
            }
            ...
            nbt.WriteUInt8(def.CollideType);
            ...
            nbt.WriteInt32(6);
            nbt.WriteUInt8((byte)(def.MinX * 16)); 
            nbt.WriteUInt8((byte)(def.MinY * 16));
            nbt.WriteUInt8((byte)(def.MinZ * 16)); 
            nbt.WriteUInt8((byte)(def.MaxX * 16));
            nbt.WriteUInt8((byte)(def.MaxY * 16)); 
            nbt.WriteUInt8((byte)(def.MaxZ * 16));
            nbt.Write(NbtTagType.End);
        }
    }
}*/