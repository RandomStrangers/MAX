...
namespace MAX
{
    ...
    public static class LevelOptions
    {
        ...
        public static List<LevelOption> Options = new List<LevelOption>() 
        {
             ...
             new LevelOption(Drawing,   SetDrawing,   "&HWhether drawing orders (e.g /z) can be used on this map."),
        ...
        {
            Toggle(p, l, ref l.Config.Drawing, "Drawing orders");
        }

        public static void SetLoadDelay(Player p, Level l, string value)
        {
            int raw = 0;
            if (!OrderParser.GetInt(p, value, "Load delay", ref raw, 0, 2000)) return;
        ...
        {
            int raw = 0;
            if (!OrderParser.GetInt(p, value, "Physics speed", ref raw, 1)) return;
        ...
        {
            int raw = 0;
            if (!OrderParser.GetInt(p, value, "Physics overload", ref raw, 500)) return;
        ...
        {
            int raw = 0;
            if (!OrderParser.GetInt(p, value, "Fall distance", ref raw)) return;
        ...
        {
            int raw = 0;
            if (!OrderParser.GetInt(p, value, "Drown time (in tenths of a second)", ref raw)) return;
