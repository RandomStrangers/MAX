...
namespace MAX
{
    ...
    public partial class Level : IDisposable
    {
        ...
        public bool Unload(bool silent = false, bool save = true)
        {
            ...
            if (cancel)
            {
                Logger.Log(LogType.SystemActivity, "Unloading of {0} canceled by a addon", name);
                return false;
            }
            MovePlayersToMain();
        ...
        public bool Save(bool force = false)
        {
            ...
            {
                path = LevelInfo.MapPath(MapName);
            }
            else
            {
        ...
        public static void LoadMetadata(Level lvl)
        {
            try
            {
                ...
                {
                    Logger.Log(LogType.MAXMessage, ".properties file for level {0} was not found.", lvl.MapName);
                }
            }
            catch (Exception e)
            {
