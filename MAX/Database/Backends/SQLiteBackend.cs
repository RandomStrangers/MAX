...
namespace MAX.SQL
{
    ...
    public class SQLiteConnection : ISqlConnection
    {
        ...
        public virtual bool ConnectionPooling { get; }
        ...
        public override ISqlOrder CreateOrder(string sql) { return new SQLiteOrder(sql, this); }
    ...
    public class SQLiteOrder : ISqlOrder
    {
        public string sqlOrd;
        ...
        public SQLiteOrder(string sql, SQLiteConnection connection)
        {
            sqlOrd = sql;
            conn = connection;
        }
        ...
        public SQLiteStatement NextStatement()
        {
            ...
            if (string.IsNullOrEmpty(sqlOrd)) return null;

            try
            {
                stmt = conn.Prepare(sqlOrd, ref sqlOrd);
            }
            catch (Exception)
            {
                DisposeStatement();
    ...
    {
        public SQLiteOrder _order;
        public SQLiteStatement stmt;
        public int readState, rowsAffected, columns;
        public string[] fieldNames;

        public SQLiteDataReader(SQLiteOrder ord)
        {
            _order = ord;
        }
        ...
        public void CheckClosed()
        {
            ...
            SQLiteConnection.Check(_order.conn);
        }
        ...
        public bool NextResult()
        {
            ...
            while (true)
            {
                stmt = _order.NextStatement(); // next statement to execute
    ...
    public class SQLiteStatement : IDisposable
    {
        ...
        public bool Step()
        {
            ...
            while (true)
            {
                ...
                // the database is locked, then keep retrying until the order timeout occurs.
    ...
    public class SQLiteTransaction : ISqlTransaction
    {
        ...
        public SQLiteTransaction(SQLiteConnection connection)
        {
            ...
            if (conn._transactionLevel++ == 0)
            {
                try
                {
                    using (ISqlOrder ord = conn.CreateOrder("BEGIN IMMEDIATE"))
                    {
                        ord.ExecuteNonQuery();
                    }
                }
        ...
        public override void Commit()
        {
            ...
            if (--conn._transactionLevel == 0)
            {
                using (ISqlOrder ord = conn.CreateOrder("COMMIT"))
                {
                    ord.ExecuteNonQuery();
                }
            }
        ...
        public void IssueRollback(bool throwError)
        {
            ...
            try
            {
                using (ISqlOrder ord = conn.CreateOrder("ROLLBACK"))
                {
                    ord.ExecuteNonQuery();
                }
            }
