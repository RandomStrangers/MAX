    ...
    public class ClassicProtocol : IGameSession
    {
        ...
        public override int HandlePacket(byte[] buffer, int offset, int left)
        {
            switch (buffer[offset])
            {
                ...
                case Opcode.CpeAddonMessage: return HandleAddonMessage(buffer, offset, left);
        ...
        public int HandleAddonMessage(byte[] buffer, int offset, int left)
        {
            const int size = 1 + 1 + Packet.AddonMessageDataLength;
            if (left < size) return 0;

            byte channel = buffer[offset + 1];
            byte[] data = new byte[Packet.AddonMessageDataLength];
            Array.Copy(buffer, offset + 2, data, 0, Packet.AddonMessageDataLength);
            OnAddonMessageReceivedEvent.Call(player, channel, data);
