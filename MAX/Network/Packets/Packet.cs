...
namespace MAX.Network
{
    ...
    public static class Packet
    {
        ...
        public const int AddonMessageDataLength = 64;
        public static byte[] AddonMessage(byte channel, byte[] data)
        {
            byte[] buffer = new byte[66];
            int i = 0;
            buffer[i++] = Opcode.CpeAddonMessage;
            buffer[i++] = channel;
            Array.Copy(data, 0, buffer, i, AddonMessageDataLength);
