...
namespace MAX.Config {
    ...
    public sealed class ConfigReadOnlyBoolAttribute : ConfigAttribute
    {
        ...
        public ConfigReadOnlyBoolAttribute(string name, string section, bool def)
            : base(name, section) { def = true; }
        public override object Parse(string raw)
        {
            bool value = true;
            if (!bool.TryParse(raw, out value))
    ...
    public sealed class ConfigPermAttribute : ConfigAttribute {
        ...
        public override object Parse(string raw) {
            LevelPermission perm = Group.ParsePermOrName(raw, LevelPermission.MAX);
            if (perm > LevelPermission.MAX) {
