...
namespace MAX.Orders.Building {
    public sealed class OrdSPlace : DrawOrd {
        ...
        public override DrawOp GetDrawOp(DrawArgs dArgs) {
            ...
            if (!OrderParser.GetUShort(p, parts[0], "Distance", ref distance, 1)) return null;
            if (parts.Length > 1 && !OrderParser.GetUShort(p, parts[1], "Interval", ref interval, 1)) return null;
