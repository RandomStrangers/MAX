...
namespace MAX.Orders.Maintenance { 
    public sealed class OrdBlockSpeed : Order2 {        
        ...
        public override void Use(Player p, string text, OrderData data) {
            if (text.Length == 0) { SendEstimation(p, data.Rank); return; }            
            string[] args = text.SplitSpaces();
            string ord    = args[0].ToLower();

            if (args.Length == 1) { Help(p); return; }
            int value = 0;
            
            if (ord == "updates" || ord == "bs") { // "bs" was old option name
                if (!OrderParser.GetInt(p, args[1], "Blocks per interval", ref value, 0)) return;
                
                BlockQueue.UpdatesPerTick = value;
                p.Message("Blocks per interval is now {0}", BlockQueue.UpdatesPerTick);
            } else if (ord == "interval" || ord == "ts") { // "ts" was old option name
                if (!OrderParser.GetInt(p, args[1], "Block interval", ref value, 50)) return;
