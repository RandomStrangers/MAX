...
namespace MAX.Modules.Moderation.Notes
{
    public class OrdNotes : Order2
    {
        ...
        public static void PrintNotes(Player p, string ord, string name, string modifier) {
            ...
            if (modifier.Length == 0) {
            	Paginator.Output(p, notes, PrintNote, ord, "Notes", 
            	                 (1 + (notes.Count - 8)).ToString());
            	p.Message("To see all Notes, use &T/{0} all", ord);
            	return;
            }
        ...
        public static string Action(string arg) {
            ...
            if (arg.CaselessEq("F")) return "Jailed";
    ...
    public class OrdMyNotes : OrdNotes
    {
