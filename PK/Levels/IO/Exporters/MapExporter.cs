...
namespace PattyKaki.Levels.IO
{
    ...
    public unsafe class MapExporter : IMapExporter
    {
        ...
        public static void WriteZonesSection(Level lvl, Stream gs, byte[] buffer)
        {
            ...
            {
                NetUtils.WriteU8((byte)z.MinX, buffer, 0 * 2); 
                NetUtils.WriteU8((byte)z.MaxX, buffer, 1 * 2);
                NetUtils.WriteU8((byte)z.MinY, buffer, 2 * 2); 
                NetUtils.WriteU8((byte)z.MaxY, buffer, 3 * 2);
                NetUtils.WriteU8((byte)z.MinZ, buffer, 4 * 2); 
                NetUtils.WriteU8((byte)z.MaxZ, buffer, 5 * 2);
                ...
                for (int i = 0; i < elem.Length; i++)
                {
                    ...
                    NetUtils.WriteU8((byte)count, buffer, 0);
