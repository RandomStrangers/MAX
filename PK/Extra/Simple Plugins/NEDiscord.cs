...
namespace Flames.Modules.Relay.NEDiscord
{
    ...
    public sealed class DiscordApiClient : AsyncWorker<DiscordApiMessage>
    {
        ...
        static void LogError(Exception ex, DiscordApiMessage msg)
        {
            ...
            Logger.LogError("Error sending request to NEDiscord API " + target, ex);
        }

        static void LogWarning(Exception ex)
        {
            Logger.Log(LogType.Warning, "Error sending request to NEDiscord API - " + ex.Message);
        }
        ...
        static void LogResponse(string err)
        {
            ...
            Logger.Log(LogType.Warning, "NEDiscord API returned: " + err);
        }
        ...
        static void SleepForRetryPeriod(WebResponse res)
        {
            ...
            Logger.Log(LogType.SystemActivity, "NEDiscord bot ratelimited! Trying again in {0} seconds..", delay);
    ...
    public sealed class DiscordBot : RelayBot
    {
        ...
        public override string RelayName { get { return "NEDiscordBot"; } }
        ...
        TextFile replacementsFile = new TextFile("text/discord/replacements_ne.txt",
                                        "// This file is used to replace words/phrases sent to NEDiscord",
        ...
        {
            Controllers = PlayerList.Load("text/discord/controllers_ne.txt");
        }
    ...
    public sealed class DiscordConfig
    {
        ...
        public const string PROPS_PATH = "properties/discordbot_ne.properties";
        ...
        public void Save()
        {
            ...
            {
                w.WriteLine("# NEDiscord relay bot configuration");
    ...
    {
        public override string Name { get { return "NEDiscordRelay"; } }
public override string Flames_Version { get { return "1.0.0.0"; } }
        ...
        public override void Load(bool startup)
        {
            ...
            Command.Register( new CmdNEDiscordBot());
			Command.Register( new CmdNEDiscordControllers());
            Bot.Config = Config;
            Bot.ReloadConfig();
            Bot.Connect();
        ...
        public override void Unload(bool shutdown)
        {
            ...
            Bot.Disconnect("Disconnecting NEDiscord bot");
            Command.Unregister(Command.Find("NEDiscordBot"));
			Command.Unregister(Command.Find("NEDiscordControllers"));
        }

        void OnConfigUpdated() { Bot.ReloadConfig(); }
    }

    public sealed class CmdNEDiscordBot : RelayBotCmd
    {
        public override string name { get { return "NEDiscordBot"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }

    public sealed class CmdNEDiscordControllers : BotControllersCmd
    {
        public override string name { get { return "NEDiscordControllers"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }
    ...
    public sealed class DiscordWebsocket : ClientWebSocket
    {
        ...
        {
            SentIdentify = false;
            if (readable) Logger.Log(LogType.SystemActivity, "NEDiscord relay bot closing: " + reason);
            Close();

            if (reason == REASON_INVALID_TOKEN)
            {
                CanReconnect = false;
                throw new InvalidOperationException("NEDiscord relay: Invalid bot token provided - unable to connect");
            }
            else if (reason == REASON_DENIED_INTENT)
            {
                ...
                CanReconnect = false;
                throw new InvalidOperationException("NEDiscord relay: Message Content Intent is not enabled in Bot Account settings, " +
        ...
        void DispatchPacket(int opcode, JsonObject obj)
        {
            ...
            else if (opcode == OPCODE_INVALID_SESSION)
            {
                ...
                Logger.Log(LogType.Warning, "NEDiscord relay: Resuming failed, trying again in 5 seconds");
                Thread.Sleep(5 * 1000);
                Identify();
            }
        }
