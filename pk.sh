...
pkdir="./"							##  Folder containing PattyKaki.exe
...
gamename="RedCraft - PattyKaki"		##  Arbitrary name of server, will not affect actual server name
gamepid="${pkdir}/f.pid"			##  If you do not know what this is, do not worry about it, for "silent" mode only.
gamelog="${pkdir}/f.log"			##  This logs everything sent to console, if started in "silent" mode
...
if [ ! -f "${pkdir}/PattyKaki.exe" ]; then
	echo -e "cannot find ${pkdir}/PattyKaki.exe!"
...
case "$1" in
	silent)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${pkdir}/PattyKaki.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			if ${monobin} ${gameopt} ${pkdir}/PattyKaki.exe 1>> ${gamelog} 2>&1 & sleep 3 ; then
				pid=`ps -ef |grep "${monobin} ${gameopt} ${pkdir}/PattyKaki.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game { print $2 } ; END {exit status}'`
				echo ${pid} > ${gamepid}
...
;;
	normal)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${pkdir}/PattyKaki.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			echo -e
			${monobin} ${gameopt} ${pkdir}/PattyKaki.exe
			if $autorestart ; then
				$0
			else
				exit 0
...
;;
	*)
			if [ -f "${pkdir}/PattyKaki.update" ]; then
				echo
				echo Update Found!
				echo -n Applying update:
				rm ${pkdir}/PattyKaki.exe
				mv ${pkdir}/PattyKaki.update ${pkdir}/PattyKaki.exe
					if [ -f "${pkdir}/PattyKaki.update" ]; then
						echo -e FAILED!
							if [ -f "${pkdir}/PattyKaki.exe" ]; then
								$0 ${default}
